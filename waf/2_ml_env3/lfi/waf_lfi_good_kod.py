...
class_flag = "bad"
output_csv_path_content_host = 'lfi_webserver_log_file.csv'

badwords = ['etc','passwd','ZXRj','L3Bhc3N3ZA==','Li4v','shadow', 'aliases',
            'anacrontab', 'apache2','at.allow', 'at.deny','bashrc', 'bootptab',
            'hosts', 'httpd','opt', 'proc','root','usr', 'lib','local','sbin', 'var','adm',
            ...
            'hostname', 'c:/']
veriler = pd.read_csv('lfi_all.csv')

x = veriler.iloc[:,0:7].values #bağımsız değişkenler
y = veriler.iloc[:,7:].values #bağımlı değişken -classes class_req
...
def request(flow: http.HTTPFlow):
	...
	sign1=raaw_path.count ("../") 
	sign2=raaw_path.count ("..\\")
	sign3=raaw_path.count (".\\.")
	sign4=raaw_path.count ("..\\..")
	sign5=raaw_path.count ("....\\")
	sign6=raaw_path.count ("..../")
