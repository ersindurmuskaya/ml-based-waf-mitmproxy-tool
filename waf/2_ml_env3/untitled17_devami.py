...
class MitmProxyApp:
	def __init__(self, master):
		...
		self.start_button = ttk.Button(master, text="Start MitmProxy", command=self.start_mitmproxy)
		self.start_button.pack(pady=10)

		# Stop button to stop mitmproxy process
		self.stop_button = ttk.Button(master, text="Stop MitmProxy", command=self.stop_mitmproxy, state=tk.DISABLED)
	...
	def start_mitmproxy(self):
		...
		mitm_script_path = 'kaggle_xss_script.py'  # Change this to your actual script file
		mitm_proxy_port = 8080		# İstediğiniz port numarasını burada belirtin
		mitm_proxy_ip = '0.0.0.0'
		command = ['mitmdump', '-s', mitm_script_path, '-p', str(mitm_proxy_port), '--listen-host', f'{mitm_proxy_ip}' ]		  
