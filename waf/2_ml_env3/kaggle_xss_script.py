...
badwords = ['script','src', 'alert', 'onload', 
			...
			'object', 'title', 'onLoad', 'frameset ', 'style', 'applet', 'xml', 
...
def request(flow: http.HTTPFlow):
	...
	if host_site=='192.168.174.135' or  'localhost':
		...
		path2=request_path #mitmproxy de content olan
		#print("path2 : : : :" , path2)
...
def response(flow: http.HTTPFlow):
	# Eğer HTTP yanıtı hakkında bilgi almak isterseniz burada islemler yapabilirsiniz
	pass

if __name__ == "__main__":
	from mitmproxy.tools.main import mitmdump
	mitmdump(['-s', __file__, '-p', '8080','--listen-host','0.0.0.0'])  # Burada da mitmproxy nin portunu belirtin
	#mitmdump(['-s', __file__])
