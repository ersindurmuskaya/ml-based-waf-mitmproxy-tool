...
y = veriler.iloc[:,8:].values.ravel() #bağımlı değişken
...
feature_names_osc = veriler.iloc[:,0:8].columns
class_names_lfi = veriler.iloc[:,8:].columns
...
_ = tree.plot_tree(en_iyi_model, 
                   feature_names=feature_names_osc ,  
                   class_names=["bad","good"],
                   fontsize=20,
                   filled=True)
